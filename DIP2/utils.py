...
def getSpectrum(img):
    img_dft = cv2.dft(np.float32(img), flags=cv2.DFT_COMPLEX_OUTPUT|cv2.DFT_SCALE)
...
def convolute(spec1, spec2):
    return cv2.mulSpectrums(spec1, spec2, flags=cv2.DFT_COMPLEX_OUTPUT|cv2.DFT_SCALE)
